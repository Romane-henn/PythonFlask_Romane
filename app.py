...
# --- MODULE DBSCAN (DENSITÉ & SURFACE REJETÉE) ---
...
def dbscan():
    ...
    result_image = None
    stats = {"k": 0, "bruit_pixels": 0, "total_pixels": 0, "surface_rejetee": 0}
    details_classes = []
    ...
    if photo and dossier:
        ...
        n_clusters = len([l for l in labels_set if l != -1])
        n_bruit = np.count_nonzero(labels == -1)
        total_pixels = len(labels)
        
        stats = {
            "k": n_clusters,
            "bruit_pixels": n_bruit,
            "total_pixels": total_pixels,
            "surface_rejetee": round((n_bruit / total_pixels) * 100, 2)
        }
        ...
        for i in labels_set:
            mask = (labels == i)
            if i == -1:
                new_pixels[mask] = [0, 0, 0] # Surface rejetée en noir
            else:
                couleur_moy = np.mean(pix_flat[mask], axis=0).astype(int)
                new_pixels[mask] = couleur_moy
                details_classes.append({
                    "id": i,
                    "couleur": f"rgb({couleur_moy[0]},{couleur_moy[1]},{couleur_moy[2]})",
                    "taille": np.count_nonzero(mask)
                })
    ...
    return render_template("dbscan.html", images=images, dossier=dossier, result_image=result_image, 
                           photo_origine=photo, eps=eps, min_samples=min_samples, stats=stats, details=details_classes)
